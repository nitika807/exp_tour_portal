	...
	if(!isset($_GET['my_orders'])){
		if(!isset($_GET['edit_account'])){
			if(!isset($_GET['change_pass'])){
				...
				if($count_orders > 0){
					echo "<h3 style='color: red;' >Important!</h3>
							<h4>You have ($count_orders) pending order(s).</br>
							Please see your order details by clicking <a href ='my_account.php?my_orders'> here </a>! </br>
							<b><h3> Or </h3></b> You can pay <a href = '../checkout.php'>offline</a></h4>";
							
					
				}
				else{
						echo "<h3 style='color: red;' >Important!</h3>
							<h4>You have no pending order(s).</br>
							Please see your order history by clicking <a href ='my_account.php?my_orders'> here </a>! ";
				}
					}
				}
			}
...
function cart(){
	...
	if(mysqli_num_rows($run_check)>0){
		...
		echo "Tour Booked. Continue searching this site to book more tours..";
	}
	}
 ...
 <?php 
		...
		//getting categories
								...
								while ($row_cats = mysqli_fetch_array($run_cats)){
									...
									echo "<li><span><a href = 'offers.php?cat= $cat_id'>$cat_title</a></span></li>";
								}
							}
								function getCatPost(){
									global $db;
								if(isset($_GET['cat'])){
									$cat_id = $_GET['cat'];
									$cat_query = "select * from post where cat_id = '$cat_id'";
...
function total_price(){
	...
	echo "$".$total;
								...
								function search(){
									...
									$query = "select * from post where post_keywords like '%$user_keyword%'";
									$run = mysqli_query($db, $query);
									while($row = mysqli_fetch_array($run)){
